...
void processMetaEvent(MidiFile& midifile, int i, Array<double>& event) {
   ...
   switch (data[1]) {
      case 0x51:  // tempo change
         // 51 03 aa bb cc
         // aabbcc = microseconds per quarter note
         value = data[3];
         value = (value << 8) | data[4];
         value = (value << 8) | data[5];
         cout << "%tempo = " << 60.0 * 1000000.0 / value << " BPM";
         cout << " at absolute tick time " << midifile.getEvent(0,i).time;
	 cout << endl;
         if (midifile.getEvent(0,i).time != 0) {
            cout << " (warning: does not start at time zero so";
            cout << " time value are not correct)";
	    cout << endl;
         }
         break;
...
// md5sum: 0ef8ad2a4cf65654df6f4022af9fd338 mid2mat.cpp [20100703]
